...
function CourseCard({ course, onTrack }) {
    return (
        <div className="bg-white p-4 shadow rounded-xl border border-gray-200">
            ...
            <Button
                className="mt-2 text-xs px-3 py-1 bg-red-100 text-red-600 rounded-full"
...
export default function DisrumLanding() {
    ...
    const handleOnboardResponse = async () => {
        if (onboardStep === 2 && preferences.level && preferences.topics.length) {
            try {
                await supabase.from("onboarding").insert([{ user_id: user?.id, preferences }]);
    ...
    const handleTrackProgress = async (course) => {
        try {
            await supabase.from("progress").upsert([
                { user_id: user?.id, course_id: course.id, progress: course.progress }
            ]);
            alert(`Progress for '${course.title}' saved.`);
        } catch (error) {
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {courses.map((course, index) => (
                            <CourseCard key={index} course={course} onTrack={handleTrackProgress} />
                        ))}
                    </div>
                </div>
            </section>
